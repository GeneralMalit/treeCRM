...
exports.coreDataRouter = void 0;
const express_1 = __importDefault(require("express"));
const requireAuth_1 = require("../middleware/requireAuth");
const requireRole_1 = require("../middleware/requireRole");
const supabaseClient_1 = require("../services/supabaseClient");
const CASE_STATUSES = ["Open", "In Progress", "Resolved", "Dropped"];
const CASE_PRIORITIES = ["Low", "Medium", "High"];
const MESSAGE_TYPES = ["text", "internal_note", "system"];
const ENDORSEMENT_STATUSES = ["Pending", "Accepted", "Rejected", "Cancelled"];
const ROLES = ["CSR", "Manager", "Executive", "Admin", "Customer"];
const router = express_1.default.Router();
router.use(requireAuth_1.requireAuth, (0, requireRole_1.requireRole)("Admin"));
...
function readUuid(value, fieldName, required = true) {
...
function readBoolean(value, fieldName, required = true) {
    if (typeof value === "undefined" || value === null) {
        return required ? { error: `${fieldName} is required.` } : { data: undefined };
    }
...
function readEnum(value, fieldName, allowedValues, required = true) {
    if (typeof value === "undefined" || value === null) {
        return required ? { error: `${fieldName} is required.` } : { data: undefined };
    }
...
function parseCustomerUpdate(body) {
...
function parseTagCreate(body) {
    ...
    if ("error" in affectsNodeColor)
        return affectsNodeColor;
    return {
        data: {
            name: name.data,
...
function parseCaseTagCreate(body) {
...
function parseCaseTagUpdate(body) {
...
function parseMessageUpdate(body) {
...
function parseEndorsementUpdate(body) {
...
function parseNotificationCreate(body) {
    const userId = readUuid(body.userId, "userId");
    if ("error" in userId)
        return userId;
...
async function fetchSingleRecord(table, id) {
    if (!supabaseClient_1.hasSupabaseAdmin || !supabaseClient_1.supabaseAdmin) {
        throw new Error("Supabase client unavailable.");
    }
    const { data, error } = await supabaseClient_1.supabaseAdmin.from(table).select("*").eq("id", id).maybeSingle();
    return { data, error };
}
...
router.patch("/users/:id", async (req, res) => {
    ...
    const nextEmail = typeof parsed.data.email === "string" ? parsed.data.email : String(existingUser.email ?? "");
