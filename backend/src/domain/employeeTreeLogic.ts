...
export function aggregatePerformanceMetrics(metricRows: PerformanceMetrics[]): PerformanceMetrics {
...
export function buildManagerCsrAssignments(
  managers: UserRow[],
  csrs: UserRow[],
...
export function parseCasePatchBody(
  body: unknown,
): ValidationResult<{ status?: CaseStatus; priority?: CasePriority }> {
  if (!isRecord(body)) {
...
export function parseTagUpdateBody(
  body: unknown,
): ValidationResult<{ tagIds: string[]; customTagNames: string[] }> {
  if (!isRecord(body)) {
    return { error: "Request body must be a JSON object." };
  }
...
export function parseEndorseCaseBody(body: unknown): ValidationResult<{ endorsedToId: string }> {
  if (!isRecord(body)) {
    return { error: "Request body must be a JSON object." };
  }
...
export function parseEndorsementDecisionBody(
  body: unknown,
): ValidationResult<{ status: EndorsementDecision }> {
  if (!isRecord(body)) {
    return { error: "Request body must be a JSON object." };
  }
...
export function parseCaseReassignBody(
  body: unknown,
): ValidationResult<{ assigneeId: string; reason?: string }> {
  if (!isRecord(body)) {
    return { error: "Request body must be a JSON object." };
  }
...
export function canAccessCase(viewer: { role: Role; sub: string }, caseItem: CaseRow): boolean {
  if (viewer.role === "CSR") {
